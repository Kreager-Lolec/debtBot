...
listpersonChallenge = ['kreager', 'mihailik_panchuk', 'a111111178012648', 'J7AD3EAKTQS']
...
def CreateChat(ChatId):
    mycursor = mydb.cursor()
    mycursor.execute(f'''
        INSERT INTO CHATS (ChatId, CountUsers, DebtLimitValue)
        SELECT {ChatId}, {0}, {DebtLimitValue}
...
def InsertDeletedData():
    ...
    mycursor.execute(f'''
            INSERT INTO CHATS (ChatId, CountUsers, DebtLimitValue)
            SELECT {-792145823}, {3}, {DebtLimitValue}
        WHERE 
            NOT EXISTS ( SELECT * FROM CHATS WHERE ChatId = {-792145823}) ''')
    mydb.commit()
    mycursor.execute(f'''
            INSERT INTO CHATS (ChatId, CountUsers, DebtLimitValue)
            SELECT {-739249390}, {2}, {DebtLimitValue}
        WHERE 
            NOT EXISTS ( SELECT * FROM CHATS WHERE ChatId = {-739249390}) ''')
    mydb.commit()
    mycursor.execute(f'''
        INSERT INTO PERSONS (PersonId, ListChatId, Name, UserName)
        SELECT {256266717},N'-792145823;-739249390;',N'Дмитро Журанський Вікторович',N'kreager'
        WHERE
    ...
    insert_query_card = f'''
        INSERT INTO CARDS (CardValue,PersonId,ChatId)
        SELECT 'Моно : 4441 1144 4405 4843{backslach}Приват : 4149 6090 0004 7110',{256266717},{-792145823}
