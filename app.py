...
def changedebtlimit(message):
    if CheckUser(message.from_user.id, message.chat.id):
        if CheckIfChatHaveVoting(message.chat.id):
            ...
            purposeOfVoting = ['боргу', 'грн', getDebtLimitValue(message.chat.id)]
...
def changetimelimit(message):
    if CheckUser(message.from_user.id, message.chat.id):
        if CheckIfChatHaveVoting(message.chat.id):
            ...
            markup.row(item3)
            msg = bot.reply_to(message, 'Вкажіть новий ліміт ( число )', reply_markup=markup)
            purposeOfVoting = ['часу', 'хв', getTimeLimitValue(message.chat.id)]
            bot.register_next_step_handler(msg, vote, userName, purposeOfVoting)
    else:
        bot.reply_to(message, f'Для початку увійдіть в гільдію: команда /entertheparty')
    ...
    if userName == message.from_user.username:
        if message.text == "🛑 Відмінити операцію!":
            ...
            if number == getDebtLimitValue(message.chat.id):
                ...
                msg = bot.reply_to(message,
                                   f"Користувач @{userName} хоче встановити новий ліміт {purposeOfVoting[0]}: {number} {purposeOfVoting[1]} при поточному - {purposeOfVoting[2]} {purposeOfVoting[1]}. Прошу проголосувати. Рішення буде ухвалене або відхилене за наявності абсолютної більшості ( {str(floor(int(getCountOfActiveUsers(message.chat.id)) * 0.5) + 1)} )",
                                   reply_markup=markup)
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("noгрн"))
def callback_query(call: types.CallbackQuery):
    try:
        markup = types.InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("yesхв"))
def callback_query(call: types.CallbackQuery):
    try:
        markup = types.InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(
    ...
    if CheckUser(call.from_user.id, call.message.chat.id):
        if checkIfPersonVotes(call.from_user.id, call.message.chat.id):
            ...
            if getVotesYesByChatId(call.message.chat.id) == floor(
                    int(getCountOfActiveUsers(call.message.chat.id)) * 0.5) + 1:
                msg = bot.send_message(call.message.chat.id,
                                       f"Рішення ухвалено. Голосів за: {getVotesYesByChatId(call.message.chat.id)} / Голосів проти: {getVotesNoByChatId(call.message.chat.id)}, тепер новий ліміт часу буде становити {getPurposeByChatId(call.message.chat.id)} хв")
                ...
                bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
                    ...
                    call.message.chat.id) == getCountOfActiveUsers(call.message.chat.id):
                msg = bot.send_message(call.message.chat.id,
                                       f"Рішення не ухвалено через рівну кількість голосів. Голосів за: {getVotesYesByChatId(call.message.chat.id)} / Голосів проти: {getVotesNoByChatId(call.message.chat.id)}, ліміт часу залишатиметься {getTimeLimitValue(call.message.chat.id)} хв")
                deleteVoting(msg.chat.id)
                bot.pin_chat_message(chat_id=msg.chat.id, message_id=msg.message_id, disable_notification=True)
                bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
    else:
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("noхв"))
def callback_query(call: types.CallbackQuery):
    try:
        markup = types.InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(
    ...
    if CheckUser(call.from_user.id, call.message.chat.id):
        if checkIfPersonVotes(call.from_user.id, call.message.chat.id):
            ...
            if getVotesNoByChatId(call.message.chat.id) == floor(
                    int(getCountOfActiveUsers(call.message.chat.id)) * 0.5) + 1:
                msg = bot.send_message(call.message.chat.id,
                                       f"Рішення не ухвалено. Голосів проти: {str(getVotesNoByChatId(call.message.chat.id))} / Голосів за: {str(getVotesYesByChatId(call.message.chat.id))}, ліміт часу залишатиметься {getTimeLimitValue(call.message.chat.id)} хв")
                deleteVoting(msg.chat.id)
                bot.pin_chat_message(chat_id=msg.chat.id, message_id=msg.message_id, disable_notification=True)
                bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
            elif getVotesYesByChatId(call.message.chat.id) + getVotesNoByChatId(
